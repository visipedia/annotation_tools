...
export class EditSequence extends React.Component {
  constructor(props) {
    super(props);
    const startId = props.startId; // | null;
    this.state = {
      imageIndex: -1,
      fetchingData: true
    };
    ...
    if (startId) {
      for (let i = 0; i < props.imageIds.length; i++)
        if (startId == props.imageIds[i]) this.state.imageIndex = i;
    }
  ...
  componentDidMount() {
    ...
    let startIndex = this.state.imageIndex == -1 ? 0 : this.state.imageIndex;
    if (this.props.imageIds.length > 0) {
      let nextImageId = this.props.imageIds[startIndex];
      ...
      <div>
        ...
        <nav className="navbar fixed-bottom navbar-light bg-light">
          <div className="ml-auto">
            ...
            <span>
              {" "}
              Image {current_image} / {num_images}{" "}
            </span>
          </div>
          <div className="ml-auto" />
        </nav>
