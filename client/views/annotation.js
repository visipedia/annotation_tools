...
export class Annotation extends React.Component {
  ...
  keypointVisibilityChanged(keypoint_index, visibility) {
    this.props.handleKeypointVisibilityChange(
      this.props.id,
      keypoint_index,
      visibility
    );
  ...
  render() {
    ...
    for (var j = 0; j < this.props.keypoints.length / 3; j++) {
      ...
      keypointItems.push(
        <KeypointInfo
          ...
          name={keypoint_name}
          visibility={v}
          visibilityChanged={this.keypointVisibilityChanged}
    ...
    if (num_na_keypoints > 0) {
      na_keypoints_badge = (
        <span className="badge badge-warning">{num_na_keypoints} N/A</span>
      );
    } else {
      na_keypoints_badge = (
        <span className="badge badge-success">{num_na_keypoints} N/A</span>
      );
    }
    ...
    return (
      <div className="card">
        <div
          ...
          role="tab"
          id={"annotationHeader" + this.props.id}
        ...
        >
          <div className="d-flex justify-content-between">
            <div
              ...
              data-parent="#annotationAccordion"
              href={"#annotationBody" + this.props.id}
              style={{ cursor: "pointer" }}
            >
              <span
                className="badge px-2 mr-1"
                style={{ backgroundColor: annotation_color }}
              />
              <span>{this.props.category.name}</span>
            </div>
            <div className="p-2">{na_keypoints_badge}</div>
            <div className="p-2">
        ...
        <div
          className="collapse"
          role="tabpanel"
          id={"annotationBody" + this.props.id}
        >
